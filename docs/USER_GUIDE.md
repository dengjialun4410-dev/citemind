...
系统会用原始关键词得分和语义相似度做相关性门控。如果当前论文没有直接证据，会明确提示“不作推断”，而不是从无关段落中拼凑答案。问题尽量包含论文中的具体概念；询问单篇论文时应先选中该文档。
...
正文清洗只在建立索引时执行。选择该文档并点击“重建索引”，等状态恢复为“索引就绪”后再提问。

### 系统提示“没有找到直接相关的证据”

这表示相关性门控阻止了无依据回答。先确认选中了正确论文，再把宽泛问题改成包含方法名、数据集名或具体术语的问题；如果原文确实没有该信息，系统不会替作者推断。
